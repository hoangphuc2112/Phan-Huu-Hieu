...
export class AppController {
  ...
  news() {
    return {
      ...
      // Đã sửa lại mảng articles: thêm slug vào đây
      articles: [
        {
  ...
  // --- TRANG SINGLE BLOG (CHI TIẾT BÀI VIẾT) ---
  @Get('news/:slug') // Route động, ví dụ: /news/is-web3-still-relevant
  @Render('single-blog')
  singleBlog(@Param('slug') slug: string) {
    // Dữ liệu mẫu (Sau này bạn sẽ thay bằng gọi API WordPress)
    return {
      post: {
